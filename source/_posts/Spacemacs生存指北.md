...
`SPC f t` neotree方式显示文件路径

`Ctrl s` 搜索当前文件（需安装ivy layer）

`*` 另一种搜索文件的姿势（需将光标置于需搜索的单词处）

- `n`  下一个匹配
- `N/p` 前一个匹配
- `r` 改变范围：当前屏幕，当前函数，当前buffer
- `e` 编辑所有匹配（类似于替换）
- `/` 在当前project搜索
...
`SPC b Y` 复制整个buffer的内容

`SPC b P` 将剪贴板的内容粘贴到整个buffer

`SPC Tab` 切换至上一个buffer
...
`SPC '` 打开/关闭 Eshell（需安装shell layer）

`SPC a s` 打开其它种类的Shell
...
Python用的 Spacemacs 自带的 python layer，添加了一些参数：
